...
export default function PrivacyPolicyContent({ data }: { data: Record<string, unknown> }) {
  const { language } = useLanguage()

  const contentTitle = language === 'en'
    ? data?.privacyPolicyContentTitle_en as string
    : data?.privacyPolicyContentTitle_vn as string

  const contentBody = language === 'en'
    ? data?.privacyPolicyContent_en as string
    : data?.privacyPolicyContent_vn as string
